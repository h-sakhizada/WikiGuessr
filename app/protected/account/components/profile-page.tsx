...
export default function ProfileClientPage() {
				...
				try {
					...
					if (user.avatar) {
						try {
							...
							console.log(
								"No old file to delete or error deleting:",
								error
							);
						}
					}
					...
					await editProfile({ ...user, avatar: publicUrl });
	...
	const renderBadges = () => {
		...
		const selectedBadgeIds = new Set(
			badgesData.meta?.map((m) => m.badge_id)
		);
		...
		<div className="flex-1 w-full flex flex-col gap-6 p-4 sm:p-6 max-w-2xl mx-auto">
			...
			<div className="flex flex-col sm:flex-row gap-4">
				<Button
					className="flex-1"
					onClick={() =>
						(window.location.href = "/protected/statistics")
					}
				>
